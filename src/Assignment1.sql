...
select subquery.userid,subquery.created_date as FirstPurchasedDate,subquery.product_name from (
select users.userid,created_date,product_name,row_number() over(partition by sales.userid order by created_date) as Rno
from users join sales on users.userid=sales.userid join product on product.product_id=sales.product_id) as subquery where subquery.Rno=1;
